...
- 메모리 계층 : 레지스터 > 캐시 (L1/L2) > 메인 메모리 (RAM) > 보조 기억 장치 (HDD/SSD)

- 메모리 할당 방식 : 가변 분할 방식(연속 메모리 할당, Segmentation), 고정 분할 방식(비연속 메모리 할당, Paging)

- 스와핑 (Swapping) : Swap-in, Swap-out

- 외부 단편화

- 예제 : Reference Count, Garbage Collection

### 2. 가상 메모리 관리

가상 메모리는 메모리 관리를 위한 하나의 시스템이자 기술

- Paging (페이징) : 메모리 공간을 일정한 크기로 나누어 다루는데 이 공간들을 페이지

- 페이징과 메모리 : 논리 주소 공간과 물리 주소 공간

- Page Table (페이지 테이블) : 페이지 번호와 프레임 번호를 짝지어주는 테이블, 논리 주소에 맞는 물리 주소를 찾아 갈 수 있도록 각 프로세스마다 Page Table (페이지 테이블)을 가지도록 한다.

- Page Table Entry (페이지 테이블 엔트리) : 페이지 테이블의 각 행 (페이지 번호, 프레임 번호, 접근비트, 변경비트, 유효비트, 보호비트)

- Page Segmentation (페이지 세그멘테이션) : Segmentation (가변 분할 방식)과 Paging (고정 분할 방식)을 혼합한 방식, Segmentation Table (세그멘테이션 테이블)을 관리

### 3. 페이지 교체

- 요구 페이징
: 실행할 모든 프로세스를 메모리에 올려두는 것은 시스템에 부담이 될 수 있으므로 당장 필요한 페이지만을 메모리에 우선 적재하는 방법

- 페이지 교체 정책
: 메인 메모리의 빈 공간을 만들어 주기 위해 메인 메모리에 있는 페이지 중 어떤 페이지를 스왑 영역으로 보낼 것인지 결정 및 처리하는 작업 (선입 선출 방식, 최적 페이지 교체 방식, LRU 방식)

- Thrashing (스레싱)
: 프로세스가 실제 실행되는 시간보다 페이징에 더 많은 시간이 소요되어 성능이 떨어지는 문제
...
### 1. 파일
: 파일이란 의미있는 정보들을 한 덩어리로 모아 둔 논리적 단위로써 보조기억장치에 저장 (실행될 때는 메인 메모리를 사용)

- 파일의 구성요소
: 파일 이름, 파일 실행에 필요한 정보, 메타데이터 (데이터에 대한 데이터, 파일의 속성 - 크기, 생성된 날짜, 위치 등)

- 파일 유형 (확장자)
: 파일을 실행할 때 운영체제가 확장자를 참조해서 파일을 어떻게 실행시킬지 결정, '파일 시스템'이라는 모듈을 가지고 있고 파일 시스템은 '파일 관리자'라고도 함.

- 시스템 호출
: 파일을 다루는 모든 작업은 운영체제에 의해 이루어짐. (파일 생성 / 열기 / 읽기 / 쓰기 / 닫기 / 삭제)

### 2. 디렉터리
: 운영체제는 파일을 정돈할 수 있도록 디렉터리를 지원. 디렉터리는 한 개 이상의 파일을 가질 수 있고, 한 개 이상의 디렉터리도 가질 수 있다.

- 경로 : 절대 경로, 상대 경로

- 디렉터리와 파일
: 일반 파일에는 데이터가 저장되어 있지만 디렉터리에는 파일 정보가 저장.
디렉터리에 저장된 파일 정보를 엔트리. 각각의 엔트리에는 디렉터리에 포함된 파일의 이름과 메모리 주소는 필수적으로 포함.

### 3. 파일과 메모리
: 파일 시스템에서는 메모리 공간을 일정한 크기의 공간인 Block(블록)단위로 나누고 주소를 각각 할당해 파일을 읽고 쓰는데 사용.

- 연속 할당 방식
- 불연속 할당 방식 : 연속 할당 (연결 리스트), 인덱스 할당
...
- bite(비트)
- 정수 표현 방법 : 부호 비트 + 2진수
- 실수 표현 방법 : 부호 비트 + 지수부 + 가수부
- 문자 표현 방법 : 문자 셋 (Character set) - ASCII Code, UTF-8, UTF-16, EUC-KR 등
